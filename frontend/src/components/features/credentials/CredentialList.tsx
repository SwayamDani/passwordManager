...
export default function CredentialList({
  ...
  const [clearPasswords, setClearPasswords] = useState<Record<string, string>>({});
  
  // Ensure we have all passwords in cleartext format
  // This is necessary because password visibility toggle doesn't
  // need to make additional API requests - passwords are already available
  useEffect(() => {
    // Initialize clear passwords dictionary with the original passwords
    const initialPasswords: Record<string, string> = {};
    Object.entries(accounts).forEach(([service, account]) => {
      initialPasswords[service] = account.password;
    });
    setClearPasswords(initialPasswords);
  }, [accounts]);
          ...
          <List>
            {filteredServices.map(service => {
              ...
              return (
                <Box key={service}>
                  <ListItem
                    onClick={() => onCredentialCardOpen(service, account.username, clearPasswords[service])}
                      ...
                      <Stack direction="row" spacing={1}>
                        <IconButton 
                          ...
                          onClick={(e) => {
                            ...
                            onCopyToClipboard(account.username, clearPasswords[service], service);
                          }}
                        ...
                        <Box sx={{ display: 'block' }}>
                          ...
                          <Box sx={{ display: 'block', mb: 1 }}>
                            <Typography 
                              ...
                              variant="body2" 
                            >
                              Password: {visiblePasswords[service] ? clearPasswords[service] : '••••••••'}
                              <IconButton
                                size="small"
                                onClick={(e) => togglePasswordVisibility(service, e)}
                                sx={{ ml: 1 }}
