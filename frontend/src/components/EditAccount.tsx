...
export default function EditAccount({ open, onClose, onAccountUpdated, service, initialData }: EditAccountProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate master password if updating password
    if (password && !masterPassword) {
      setError('Master password is required when changing the account password');
      return;
    }
    
    try {
      // Remove leading slash to avoid duplicate slashes in URL
            ...
            {password && (
              <>
                ...
                <TextField
                  ...
                  helperText="Required for encrypting your new password"
                  sx={{ mt: 2 }}
                />
              </>
            )}
